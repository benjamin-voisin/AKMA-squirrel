...
global lemma
  ...
  Let msg3 = enc(diff(make3(nb,na),len3),r3,pub skb) in
  Let fr =
    fun (f: _ -> _ -> _ -> _ -> _ -> _ -> _ ->_ -> message*bool*message) =>
      (f (pub skb) msg1 msg2 msg3 ska na nb r3')#1
  in
  Let ex =
    fun (f: _ -> _ -> _ -> _ -> _ -> _ -> _ -> _ -> message*bool*message) =>
      (f (pub skb) msg1 msg2 msg3 ska na nb r3')#2
  in
  Let pk =
    fun (f: _ -> _ -> _ -> _ -> _ -> _ -> _ -> _ -> message*bool*message) =>
      (f (pub skb) msg1 msg2 msg3 ska na nb r3')#3
  in
  [happens(t)] -> [t < B] ->
  Exists (f:_[adv]),
  [f (pub skb) msg1 msg2 msg3 ska na nb r3' =
   (frame@t,exec@t,if A<=t then input@A)].
Proof.
  intro *.
  induction t.
  + rewrite /frame /exec.
    exists (fun (a,b,c,d,e,f,g,h:message) => (zero,true,zero)).
    by rewrite if_false.
  + destruct IH.
    rewrite /*.
    rewrite and_true_r.
    exists (fun (p,m1,m2,m3,ka,na,nb,rand3:message) =>
      (<fr f, <of_bool (ex f), if ex f then <pub ka, p>>>,
       ex f,
       pk f)).
  ...
  + destruct IH.
    ...
    exists  (fun (p,m1,m2,m3,ka,na,nb,rand3:message) =>
      (<fr f,
        <of_bool (ex f),
         if ex f then
           if att (fr f) = pub skb then m1 else
           enc(make1(na,pub ska),r1',att (fr f))>>,
       ex f,
       att (fr f))).
  ...
  + destruct IH.
    ...
    rewrite and_true_r.
    exists (fun (p,m1,m2,m3,ka,na,nb,rand3:message) =>
      (<fr f, <of_bool (ex f), if ex f  then
            (if att (fr f) = m2 then (if pk f = pub skb then <m3,<na,nb>>) else
             if get2_na (dec (att (fr f), ka)) = na &&
                get2_id (dec (att (fr f), ka)) = pk f then
               <enc
                  (make3 (get2_nb (dec (att (fr f), ka)), na),
                   rand3,
                   pk f),
                if pk f = pub skb then <na,nb>>)>>,
        ex f,
        pk f)).
    assert A < A1 by use depends_A_A1.
    assert (A <= A1) <=> (A <= pred A1) as -> by auto.
    rewrite /fr /ex /pk /= H1 /input. reduce. reduce ~flags:[diffr].
...
global lemma [NSL/left,NSL_a/left] deduction_left (t:timestamp[const]) :
  ...
  Let fr =
    fun (f: _ -> _ -> _ -> _ -> _ -> _ -> _ -> message*bool*message) =>
      (f (pub ska) msg1 msg2 msg3 skb nb r2')#1
  in
  Let ex =
    fun (f: _ -> _ -> _ -> _ -> _ -> _ -> _ -> message*bool*message) =>
      (f (pub ska) msg1 msg2 msg3 skb nb r2')#2
  in
  Let pk =
    fun (f: _ -> _ -> _ -> _ -> _ -> _ -> _ -> message*bool*message) =>
      (f (pub ska) msg1 msg2 msg3 skb nb r2')#3
  in
  [happens(t)] -> [t < A1] ->
  Exists (f:_[adv]),
  [f (pub ska) msg1 msg2 msg3 skb nb r2' =
   (frame@t,exec@t,if A<=t then input@A)].
Proof.
  intro *.
  induction t.
  + rewrite /frame /exec.
    exists (fun (a,b,c,d,e,f,g:message) => (zero,true,zero)).
    by rewrite if_false.
  + destruct IH.
    rewrite /*.
    rewrite and_true_r.
    exists (fun (p,m1,m2,m3,kb,nonceb,rand2:message) =>
      (<fr f, <of_bool (ex f), if ex f then <p, pub kb>>>, ex f, pk f)).
    assert (A <= PUB) <=> (A <= pred PUB) as -> by auto.
  ...
  + destruct IH.
    ...
    exists (fun (p,m1,m2,m3,kb,nonceb,rand2:message) =>
      (<fr f, <of_bool (ex f), if ex f  then
          if att (fr f) = m1 then m2 else
          if att (fr f) = m3 then empty else
          if check_tag (dec (att (fr f), skb)) then
             enc (make2
                  (get1_na (dec (att (fr f), kb)),
                   nonceb, pub kb),
                rand2,
                get1_id (dec (att (fr f), kb)))
           >>, ex f, pk f)).
    rewrite /* in *. simpl.
...
Proof.
  ...
  + rewrite /*.
    ...
    assert frame@pred B = (f (pub skb) msg1 msg2 msg3 ska na nb r3')#1 as ->
      by rewrite Hf.
...
Proof.
  ...
  + assert A < A1 by use depends_A_A1.
    ...
    clear Hf.
    destruct Hap as [f Hf].
    assert frame@pred A1 = (f (pub ska) msg1 msg2 msg3 skb nb r2')#1 as ->
      by rewrite Hf.
    assert att (frame@pred A) = (f (pub ska) msg1 msg2 msg3 skb nb r2')#3 as ->
