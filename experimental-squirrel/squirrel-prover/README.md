...
This directory contains all the needed material to install and run Squirrel.
Details can be found below.


Also, the directory `case-studies-ccs` contains our set of 
case studies files. They include proofs of protocols with cryptographic hypotheses
supported by legacy tactics:

- `basic-hash.sp`: unlinkability of the protocol Basic-Hash
  using the EUF-MAC and PRF hypotheses;
- `global-cpa.sp`: protocol-level handling of indistinguishabilities
  using the IND-CPA game;
- `hash-lock.sp`: strong secrecy in the Hash-Lock protocol
  using the PRF axiom.

We also include two proofs using new cryptographic hypotheses
(not previously supported by Squirrel):

- `private-authentication.sp`: anonymity in the Private Authentication
  protocol under the CCA$ hypothesis;
- `nsl.sp`: equivalence of NSL protocol with its idealization using
  IND-CCA2 (main step in proving strong secrecy of nonces).
